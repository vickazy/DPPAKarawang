...
<?php
  ...
  $selectjabatan = mysqli_query($koneksi, "SELECT jabatan FROM pegawai WHERE nip='$nip'");
  $rowselect = mysqli_fetch_array($selectjabatan);
  $jabatanpegawai = $rowselect['jabatan'];
  if ($jabatanpegawai == 'PANITERA' || $jabatanpegawai == 'SEKRETARIS' || $jabatanpegawai =='KETUA') {
    ?>
    <div class="menu_section">
    <h3>Menu Khusus</h3>
    <ul class="nav side-menu">
